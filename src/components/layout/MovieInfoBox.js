...
export class MovieInfoBox extends Lightning.Component {
    ...
    return {
      ...
      Top: {
        ...
        Metadata: {
          ...
          MetadataBTM: {
            flex: { direction: 'row' },
            ReleaseDate: { text: { fontSize: 24, text: '' } },
            Language: { text: { fontSize: 24, text: '' } },
          },
          BTNS: {
            flex: { direction: 'row' },
            AddToFavBtn: { type: Button, label: '' },
            SimilarMoviesBTN: { type: Button, label: 'view similar movies' },
          },
        },
        Poster: {
          rect: true,
  ...
  _init() {
    ...
    this.tag('Bottom.RatingAndStars.Stars').text.text =
      this.movieInfoBox.vote_average + '⭐' + ' out of ' + this.movieInfoBox.vote_count + ' votes'
    this.tag('Overview').text.text = this.movieInfoBox.overview
    this.tag('Top.Metadata.MetadataBTM.Language').text.text =
      ' | ' + this.movieInfoBox.original_language
    this.tag('Top.Metadata.MetadataBTM.ReleaseDate').text.text =
      'released: ' + this.movieInfoBox.release_date
